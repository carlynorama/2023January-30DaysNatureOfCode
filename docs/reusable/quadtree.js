...
//------------------------------------------------------------------------------- ADDING
 ...
 addPoint(point, pointSuccess) {
   ...
   if (!this.containsLocation(point.x, point.y)) {
     //console.log(x,y,"not in here", this.bounds.pretty())
     return false;
   } else {
     if (this.subTrees.length != 0) {
       return this.addPointToSubTree(point,pointSuccess);
     }
 ...
 addPointToSubTree(point,pointSuccess) {
   if (this.subTrees.length == 0) {
   ...
   for (let i = 0; i < 4; i++) {
     ...
     if (this.subTrees[i].containsLocation(point.x,point.y)) {
       ...
       let result = this.subTrees[i].addPoint(point,pointSuccess);
       //console.log(result);
       return result;
     }
   }
...
//-------------------------------------------------------------------------------- ACTS ON DATA
 ...
 //   if (!(typeof(queryBounds.origin.x) === 'number' && typeof(queryBounds.origin.y) === 'number')) {
 ...
 //   }
   ...
   doWithPointsInRadius(x, y, r, myAction) {
     let rqueryBounds = Bounds.createBoundsFromCenter(x, y, r*2, r*2);
...
//----------------------------------------------------------------------------------------- EDITING TREE 
 ...
 static popCircularRegion(queryBounds, parent, owner) {
   ...
   let newPointCollector = [];
   //console.log(parent.bounds.pretty());
   if (parent.subTrees.length > 0) {
     //console.log("going deeper");
     for (let subtree of parent.subTrees) {
     ...
     let thesePoints = parent.points.filter(val=> queryBounds.inscribedCircleContains(val.x,val.y));
     for (let point of thesePoints) { owner.clearPointValue(point.x, point.y) }
     return thesePoints;
   }
   return newPointCollector;
