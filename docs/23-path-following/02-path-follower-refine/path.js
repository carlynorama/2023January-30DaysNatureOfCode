...
class Path {
    ...
    //     return false
    // }
    segmentContains(startIndex, vector) {
        ...
        let i = startIndex;
        let segmentBigBounds = Bounds.createBoundsFromPoints(this.locations[i].x - this.radius, this.locations[i].y - this.radius, this.locations[i + 1].x + this.radius, this.locations[i + 1].y + this.radius);
        if (segmentBigBounds.contains(vector.x, vector.y)) {
            ...
            if (toProject.subtracting(projection).magnitude() < this.radius) {
                return true;
            }
        }
        return false;
    ...
    segmentClosestPointTo(startIndex, vector) {
