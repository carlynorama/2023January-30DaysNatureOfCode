...
class Bounds {
    ...
    static makeBoundsFromPointArray(...components) {
    ...
    static expandedBounds(oldBounds, pointToInclude) {
        const p = new Point(Math.min(oldBounds.minX, pointToInclude.x), Math.min(oldBounds.minY, pointToInclude.y));
        const s = new Size(Math.max(oldBounds.maxX, pointToInclude.x) - p.x, Math.max(oldBounds.maxY, pointToInclude.y) - p.y);
        const b = new Bounds(p, s);
        return b;
    }
...
function minMaxOfSet(...components) {
    let x_max = -Infinity;
    let y_max = -Infinity;
    let x_min = +Infinity;
    let y_min = +Infinity;
...
function triangleContainsB(pointToTest, a, b, c) {
...
function triangleContainsA(x, y, ax, ay, bx, by, cx, cy) {
...
function triangleContainsC(pointToTest, a, b, c) {
