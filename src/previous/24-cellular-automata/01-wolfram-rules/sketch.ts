...
function drawRow(items: number[], itemSize: number) {
    for (let i = 0; i < cells.length; i++) {
    if (items[i] == 0) fill(255);
...
function generateRow(currentRow: any[], ruleSet: number) {
  let newRow = []
  for (let i = 0; i < cells.length; i++) {
...
function binaryConversion(a: number, b: number, c: any) { 
   return 0 | (a << 2) | (b << 1) | (c) 
...
const makeNumberFromArray = (array: any[]) => {
  let s = array.reduce((accumulator: any, currentValue: any) => accumulator + currentValue,
  "")
  return parseInt(s,2);
}
