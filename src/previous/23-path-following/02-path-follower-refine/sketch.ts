...
function draw() {
  ...
  if (showApparatus) { drawApparatus(pathFollower); }
...
function toggleApparatus() {
  if (showApparatus) { showApparatus = false }
  else { showApparatus = true }
}
...
function drawFollower(vehicle: DrawableVehicle) {
  //strokeWeight(2);
  push();
...
function drawTrails(wanderer: PathFollower) {
...
function drawApparatus(pathFollower: PathFollower) {
